...
server.tool("deleteAlarm", "Delete an alarm", {
    id: z.string().describe("Alarm ID"),
}, async ({ id }) => {
    await convex.mutation(api.alarm.deleteAlarm, { id });
    return {
        content: [{ type: "text", text: `Alarm ${id} deleted.` }],
    };
});
server.tool("editAlarm", "Edit alarm", {
    id: z.string().describe("Alarm ID"),
    time: z.string().optional().describe("New time"),
    label: z.string().optional().describe("New label"),
}, async ({ id, time, label }) => {
    await convex.mutation(api.alarm.editAlarm, { id, time, label });
    return {
